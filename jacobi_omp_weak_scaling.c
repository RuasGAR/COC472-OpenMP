...
int run(double *A, double *b, double *x, double *xtmp,int n_threads, int N)
{
  int itr;
  int row, col;
  double dot;
...
int main(int argc, char *argv[])
{
  ...
  FILE *file = fopen("weak_scaling.txt","a");

  int n_threads[7] = {1,2,4,8,16,32,64};
  int dimensions[7] = {500, 1000, 2000, 3000, 4000};

  for (int n_dim = 0; n_dim < 7; n_dim++)
  {
    N = dimensions[n_dim];
    ...
    for (int th = 0; th < 7; th++)
    {
      ...
      int itr = run(A, b, x, xtmp, n_threads[th], N);
